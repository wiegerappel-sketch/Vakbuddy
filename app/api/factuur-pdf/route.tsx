...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: bedrijf } = await supabase
      .from('bedrijven').select('*').eq('user_id', user.id).single()
    const { data: tarieven } = await supabase
      .from('tarieven').select('*').eq('bedrijf_id', bedrijf?.id).single()
    const { data: klus } = await supabase
      .from('klussen').select('*, klanten(*)').eq('id', klus_id).single()

    if (!bedrijf || !tarieven || !klus) {
      return NextResponse.json({ fout: 'Gegevens ontbreken.' }, { status: 400 })
    }
